...
[下载最新版 APK](https://github.com/qiq-Blanchet/YSClaude-/releases/latest) · [当前仓库](https://github.com/qiq-Blanchet/YSClaude-) · [上游项目](https://github.com/winter-bit-cry/YSClaude) · [问题反馈](https://github.com/qiq-Blanchet/YSClaude-/issues)
...
本仓库是 [winter-bit-cry/YSClaude](https://github.com/winter-bit-cry/YSClaude) 的个人维护 fork。应用主体、原始设计及大量功能来自上游项目；本 fork 当前主要增加 API 自定义请求头，以及 Qwen、ElevenLabs、MOSS 三种 TTS 接入。这是非官方自定义发行版，仅与本 fork 有关的问题请在当前仓库反馈。

## 本 fork 新增功能

| 功能 | 说明 |
| --- | --- |
| API 自定义请求头 | 每套聊天 API 配置可添加多个请求头，随模型拉取、连接测试和聊天请求发送，适合向 gateway 注入记忆开关、用户标识等信息 |
| Qwen TTS | 支持配置 Base URL、API Key、模型、音色和语言类型 |
| ElevenLabs TTS | 支持配置 Base URL、API Key、Voice ID、模型和输出格式；应用内播放建议保留默认 MP3 格式 |
| MOSS TTS | 对接 MOSI Studio 官方云服务，支持公共音色或克隆音色 ID |

原有 MiniMax、Fish Audio、Deepgram、Cartesia TTS 均保留。Qwen、ElevenLabs、MOSS 可用于普通回复朗读、AI 电台和悬浮球播报，但暂不用于实时语音通话；实时语音通话的 TTS 仍仅支持 MiniMax 或 Cartesia。MOSS 配置针对 MOSI Studio 云服务，不适用于自托管 OpenMOSS 的 OpenAI 兼容接口。

## 下载与安装

1. 打开 [Releases](https://github.com/qiq-Blanchet/YSClaude-/releases/latest)。
2. 在最新版本的 Assets 中下载 `.apk` 文件。
3. 将 APK 传到 Android 手机并打开；如系统拦截，请为当前浏览器或文件管理器开启“允许安装未知应用”。
4. 覆盖安装需要保持相同包名和签名。若提示签名冲突，请先在旧版中完成数据备份，再决定是否卸载旧版。

## API 自定义请求头

在“设置 → API 配置”中展开“自定义请求头”，上方填写请求头名称，下方填写请求头值。支持添加多组字段，值默认隐藏，可手动显示。

自定义请求头会跟随对应配置的模型拉取、连接测试和聊天请求发送。`Authorization`、`Content-Type`、`Host` 等由应用或网络层管理的字段不可覆盖。请求头值与 API Key 一样保存在本机，并包含在完整备份中；如果启用远程 Prompt Cache 保活，它们也可能发送给用户配置的保活服务。

示例：

```text
X-Memory-User: user-001
X-Memory-Enabled: true
```

## TTS 配置

在“设置 → 语音配置”的 TTS 区域选择服务商，再填写对应凭证和音色。当前普通朗读支持：
...
建议先使用“测试播放”验证配置，再保存。Qwen 的 Base URL 与 API Key 需要属于同一地域；ElevenLabs 建议使用默认 `mp3_44100_128` 输出格式；MOSS 需要填写 MOSI Studio 的 Voice ID。TTS 密钥保存在本机，并会包含在完整备份中。

## 更新记录

- 2026.07.14 / v1.1.0
  - API 配置新增多组自定义请求头，可用于 gateway 记忆注入和用户标识。
  - 普通 TTS 新增 Qwen、ElevenLabs、MOSS（MOSI Studio）服务商。
  - GitHub APK 发布使用 `preview` profile，并为 Android 构建号开启远端自动递增。
...
- Expo SDK 56：项目依赖 `expo ~56.0.15`，并按 Expo SDK 56 文档进行开发。
...
- TTS：支持 MiniMax、Fish Audio、Deepgram、Cartesia、Qwen、ElevenLabs、MOSS（MOSI Studio）播放回复。
...
入口文件 `index.js` 负责加载手势支持并进入 `expo-router/entry`；字体映射集中在 `src/theme/fonts.ts`，默认使用系统字体。
...
- 每套 API 配置可携带经校验的自定义请求头；系统管理字段不可覆盖。
...
如果 Windows PowerShell 能使用 Git，直接运行构建命令即可；如果 EAS 提示 `git command not found`，只在确认当前目录和改动正确后，在同一个窗口中先执行：
...
- `preview`：内部测试分发，Android 输出可直接安装和上传 GitHub 的 APK，并自动递增远端 `versionCode`。
- `production`：面向应用商店，默认输出 AAB，并自动递增远端 `versionCode`。
...
- `package-lock.json` 顶层与根 package 的 `version`
- `app.json` 的 `expo.version`
- `android/app/build.gradle` 的 `versionName`

仓库包含原生 `android/` 目录，因此 Android 原生配置优先。只修改 `app.json` 可能导致 APK 详情仍显示旧版本。远端 `versionCode` 由 EAS 管理，本地 `build.gradle` 中的初始值不代表已发布 APK 的实际构建号。

### 发布 GitHub APK

1. 提交并推送版本与功能改动。
2. 执行 `npm.cmd run typecheck`。
3. 使用 `preview` profile 构建 Android APK。
4. 下载并重命名为 `YSClaude-vX.Y.Z-custom.apk`。
5. 在 GitHub Releases 创建未使用过的新标签，例如 `v1.1.0-custom.2`。
6. 上传 APK、填写更新说明并发布；不要编辑或删除旧 Release，这样历史 APK 会继续保留。

## 配置入口

主要配置位于应用内设置页和 `src/stores/settings.ts`：

- API 渠道：base URL、API key、自定义请求头、模型、温度、thinking、prompt cache。
- Tool：网页搜索、网页交互、热榜、MCP、远程命令、原生设备工具、conversation artifacts。
- TTS：MiniMax、Fish Audio、Deepgram、Cartesia、Qwen、ElevenLabs、MOSS（MOSI Studio）。
